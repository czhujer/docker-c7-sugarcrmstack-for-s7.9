...
$manage_sugarcrm_files_ownership=true,
  ...
  if defined(Class['sugarcrmstack_ng']){
    if ($::sugarcrmstack_ng::sugar_version != "7.5" and $::sugarcrmstack_ng::sugar_version != "7.9"){
      fail("Class['sugarcrmstack_ng::apachephpng']: This class is compatible only with sugar_version 7.5 or 7.9 (not ${::sugarcrmstack_ng::sugar_version})")
    }
  }
  else{
